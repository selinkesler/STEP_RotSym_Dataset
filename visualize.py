...
from machining import get_machining_opt
from binvox_script_custom import create_binvox
...
def load_pretrained_model():
    ...
    ssd_net.load_weights('C:\\Users\\Z004D5TP\\PycharmProjects\\genetic\\FeatureDet3D\\weights\\VOC.pth')
...
def disp_results(filename, items):
    ...
    plotter = pv.Plotter() # BU
    # plotter = pv.Plotter(off_screen=True)
    plotter.add_mesh(mesh, opacity=0.3, color='#FFFFFF')'''

    shapetypes = ['O ring', 'Through hole', 'Blind hole', 'Triangular passage', 'Rectangular passage',
                  ...
                  'Vertical circular end blind slot', 'Horizontal circular end blind slot', '6-sides passage',
                  '6-sides pocket']

    colors = ['#000080', '#FF0000', '#FFFF00', '#00BFFF', '#DC143C', '#DAA520', '#DDA0DD', '#708090', '#556B2F',
              '#483D8B', '#CD5C5C', '#21618C', '#1C2833', '#4169E1', '#1E90FF', '#FFD700', '#FF4500', '#646464',
    ...
    for i in range(items.shape[0]):
        ...
        if flag[int(items[i, 6])] == 0:
            items[i, 1] = abs(9999-items[i, 1])
            items[i, 4] = abs(9999-items[i, 4])

            if items[i, 4] < items[i, 1]:
                tmp = items[i,4].copy()
                items[i, 4] = items[i, 1]
                items[i, 1] = tmp
            # print(items)
            '''plotter.add_mesh(pv.Cube((0, 0, 0), 0, 0, 0,
                                     (items[i, 0], items[i, 3], items[i, 1], items[i, 4], items[i, 2], items[i, 5])),
                             opacity=1, color=colors[int(items[i, 6])], style='wireframe', line_width=2,
    ...
    # get the middle point of the bounding boxes
    coord_list = []
    label_list = []
    data = []
    ...
    for i in range(items.shape[0]):
        ...
        # coord = [int((x_max + x_min) / 2), int((y_max + y_min) / 2), int((z_max + z_min) / 2)]
        # tmp = [label, int((x_max + x_min) / 2), int((y_max + y_min) / 2), int((z_max + z_min) / 2)]

        if 'hole' in label or label == '6-sides pocket' or label == '6-sides passage':
            label = 'hole'
        elif 'pocket' in label:
            label = 'pocket'

        # tmp = [label, int((x_max + x_min) / 2), -int((y_max + y_min) / 2), 10000 - int((z_max + z_min) / 2)]
        # tmp = [label, int((x_max + x_min) / 2), -(10000 - int((y_max + y_min) / 2)), int((z_max + z_min) / 2)]
        tmp = [label, int((x_max + x_min) / 2), int((y_max + y_min) / 2), int((z_max + z_min) / 2)]
        coord = [int((x_max + x_min) / 2), -(10000 - int((y_max + y_min) / 2)), int((z_max + z_min) / 2)]
        data.append(tmp)
    ...
    with open('C:\\Users\\Z004D5TP\\Desktop\\coordinates.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
    ...
    # plotter.show(screenshot='feature_image.png')
    # plotter.close()
...
def get_going():
    ...
    # filename = 'Users/Z004D5TP/Desktop/model2'
    create_binvox()
    # filename = "C:\\Users\\Z004D5TP\\Desktop\\mod\\example_model"
    # filename = 'data/MulSet/set1/2'

    labels = get_predicted_label(filename, net)  # display predicted boxes

    pred = get_lvec(labels[:, 6])

    disp_results(filename, labels)

    machining_list, id_list = get_machining_opt(pred)
    print(machining_list)
    # print(tp)

    return pred, machining_list, id_list


if __name__ == '__main__':
    get_going()
    input('press enter to continue')
    # with open("C:/Users/Z004D5TP/Downloads/Mappe1.csv") as csvdatei:
